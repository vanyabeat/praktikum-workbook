...
VariableValue::VariableValue(std::vector<std::string> dot_ids)
{
	this->var_names_ = dot_ids;
}

ObjectHolder VariableValue::Execute(Closure& closure, [[maybe_unused]] Context& context)
{
	auto* closure_p = &closure;
	for (size_t ptr = 0; ptr < this->var_names_.size(); ptr++)
	{
		if (closure_p->count(this->var_names_[ptr]) == 1)
		{
			if (ptr + 1 < this->var_names_.size())
			{
				closure_p = &((*closure_p)[this->var_names_[ptr]].TryAs<runtime::ClassInstance>()->Fields());
			}
			else
			{
				return (*closure_p)[this->var_names_[ptr]];
			}
		}
	}
	throw std::runtime_error("Execute error (Not in list)");
}
...
ObjectHolder MethodCall::Execute(Closure& closure, Context& context)
{
	...
	throw std::runtime_error("Execute error (can not call method " + method_ + ")");
}
...
ObjectHolder Add::Execute(Closure& closure, Context& context)
{
	...
	if (lhs.TryAs<runtime::Number>() != nullptr && rhs.TryAs<runtime::Number>() != nullptr)
	{
		return ObjectHolder::Own(
			runtime::Number(lhs.TryAs<runtime::Number>()->GetValue() + rhs.TryAs<runtime::Number>()->GetValue()));
	}
	else if (lhs.TryAs<runtime::String>() != nullptr && rhs.TryAs<runtime::String>() != nullptr)
	{
		return ObjectHolder::Own(
			runtime::String(lhs.TryAs<runtime::String>()->GetValue() + rhs.TryAs<runtime::String>()->GetValue()));
	}
	...
	throw runtime_error("Execute error (type add error)");
}
...
ObjectHolder Sub::Execute(Closure& closure, Context& context)
{
	...
	throw runtime_error("Execute error (type sub error)");
}
...
ObjectHolder Mult::Execute(Closure& closure, Context& context)
{
	...
	throw runtime_error("Execute error (type multi error)");
}
...
ObjectHolder Div::Execute(Closure& closure, Context& context)
{
	...
	if (lhs.TryAs<runtime::Number>() != nullptr && rhs.TryAs<runtime::Number>() != nullptr)
	{
		...
		{
			throw runtime_error("Execute error (divided by zero)");
		}
		return ObjectHolder::Own(
			runtime::Number(lhs.TryAs<runtime::Number>()->GetValue() / rhs.TryAs<runtime::Number>()->GetValue()));
	}
...
{
	auto ret = this->st_->Execute(closure, context);
...
{
	NewInstance instance(*cls_.TryAs<runtime::Class>());
	closure[cls_.TryAs<runtime::Class>()->GetName()] = instance.Execute(closure, context);
...
FieldAssignment::FieldAssignment(VariableValue object, std::string field_name, std::unique_ptr<Statement> rv)
	: str_name_(std::move(field_name)), obj_(std::move(object)), rv_(std::move(rv))
{
}
...
ObjectHolder FieldAssignment::Execute(Closure& closure, Context& context)
{
	...
	{
		throw std::runtime_error("Execute error (some data error)");
	}
...
IfElse::IfElse(std::unique_ptr<Statement> condition, std::unique_ptr<Statement> ifBody,
			   std::unique_ptr<Statement> elseBody)
	: condition_(std::move(condition)), if_block_(std::move(ifBody)), else_block_(std::move(elseBody))
{
}

ObjectHolder IfElse::Execute(Closure& closure, Context& context)
{
	if (runtime::IsTrue(this->condition_->Execute(closure, context)))
	{
		return this->if_block_->Execute(closure, context);
	}
	else
	{
		if (this->else_block_)
		{
			return this->else_block_->Execute(closure, context);
		}
	}
	return ObjectHolder::None();
}
