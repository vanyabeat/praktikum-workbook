...
Class::Class(std::string name, std::vector<Method> methods, const Class* parent)
	: class_name_(name), methods_(std::move(methods)), parrent_class_(parent)
{
}
...
bool Less(const ObjectHolder& lhs, const ObjectHolder& rhs, Context& context)
{
	...
	{
		const std::string& lhs_str = lhs.TryAs<String>()->GetValue();
		const std::string& rhs_str = rhs.TryAs<String>()->GetValue();
		return std::lexicographical_compare(lhs_str.begin(), lhs_str.end(), rhs_str.begin(), rhs_str.end());
	}
	else if (lhs.TryAs<Bool>() != nullptr && rhs.TryAs<Bool>() != nullptr)
	{
		return lhs.TryAs<Bool>()->GetValue() < rhs.TryAs<Bool>()->GetValue();
