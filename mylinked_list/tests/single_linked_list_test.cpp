...
public:
    template<typename T>
    std::vector<T> CreateUniqMergedVector(std::vector<T> &a, std::vector<T> &b) {
        std::vector<T> vec(a);
...
TEST_F(SingleLinkedListTests, AssignFromVector) {
    using namespace std;
    std::vector<std::string> vec = {"first"s, "last"s};
    std::vector<std::string> vec2 = {"begin"s, "end"s};
    SingleLinkedList<std::string> my_list(vec.begin(), vec.end());
    SingleLinkedList<std::string> my_list2{"first"s, "last"s};

    ASSERT_EQ(my_list, my_list2);
    my_list.reassign(vec2.begin(), vec2.end());
    ASSERT_EQ(my_list, SingleLinkedList({"begin"s, "end"s}));
}
...
template<class Type>
auto PushToFunction(std::vector<SingleLinkedList<Type>> &lists, int x) {
...
TEST_F(SingleLinkedListTests, AddressSanitizer) {
    using namespace std;
