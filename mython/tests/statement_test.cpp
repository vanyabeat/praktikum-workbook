...
template <typename T> void AssertObjectValueEqual(ObjectHolder obj, const T& expected, const string& msg)
{
	ostringstream one;
	...
	ASSERT_EQ(one.str(), two.str());
}


TEST(StatementTest, TestNumericConst)
{
	runtime::DummyContext context;
	...
	ostringstream os;
	o->Print(os, context);
	ASSERT_EQ(os.str(), "57"s);

	ASSERT_TRUE(context.output.str().empty());
}

TEST(StatementTest, TestStringConst)
{
	runtime::DummyContext context;
	...
	ostringstream os;
	o->Print(os, context);
	ASSERT_EQ(os.str(), "Hello!"s);

	ASSERT_TRUE(context.output.str().empty());
}

TEST(StatementTest, TestVariable)
{
	runtime::DummyContext context;
	...
	ASSERT_TRUE(VariableValue("x"s).Execute(closure, context).Get() == &num);
	ASSERT_TRUE(VariableValue("w"s).Execute(closure, context).Get() == &word);
	ASSERT_THROW(VariableValue("unknown"s).Execute(closure, context), std::runtime_error);
...
TEST(StatementTest, TestAssignment)
{
...

	{
		...
		ASSERT_TRUE(o);
		AssertObjectValueEqual(o, 57, "");
	}
	ASSERT_TRUE(closure.find("x"s) != closure.end());
	AssertObjectValueEqual(closure.at("x"s), 57, "");

	{
		ObjectHolder o = assign_y.Execute(closure, context);
		ASSERT_TRUE(o);
		AssertObjectValueEqual(o, "Hello"s, "");
	}
	ASSERT_TRUE(closure.find("y"s) != closure.end());
	AssertObjectValueEqual(closure.at("y"s), "Hello"s, "");

	ASSERT_TRUE(context.output.str().empty());
}

TEST(StatementTest, TestFieldAssignment)
{
	runtime::DummyContext context;
	...
	ASSERT_TRUE(object.Fields().find("x"s) != object.Fields().end());
	AssertObjectValueEqual(object.Fields().at("x"s), 57, "");
							  ...
							  make_unique<StringConst>(runtime::String("Hello, world! Hooray! Yes-yes!!!"s)));
	{
		...
		ASSERT_TRUE(o);
		AssertObjectValueEqual(o, "Hello, world! Hooray! Yes-yes!!!"s, "");
	}

	ASSERT_TRUE(object.Fields().find("y"s) != object.Fields().end());
	const auto* subobject = object.Fields().at("y"s).TryAs<runtime::ClassInstance>();
	ASSERT_TRUE(subobject != nullptr && subobject->Fields().find("z"s) != subobject->Fields().end());
	AssertObjectValueEqual(subobject->Fields().at("z"s), "Hello, world! Hooray! Yes-yes!!!"s, "");

	ASSERT_TRUE(context.output.str().empty());
}

TEST(StatementTest, TestPrintVariable)
{
	runtime::DummyContext context;
	...
	ASSERT_EQ(context.output.str(), "42\n"s);
}

TEST(StatementTest, TestPrintMultipleStatements)
{
	runtime::DummyContext context;
	...
	ASSERT_EQ(context.output.str(), "hello 57 Python None\n"s);
}

TEST(StatementTest, TestStringify)
{
	runtime::DummyContext context;
...

	{
		...
		AssertObjectValueEqual(result, "57"s, "");
		ASSERT_TRUE(result.TryAs<runtime::String>());
	}
	{
		auto result = Stringify(make_unique<StringConst>("Wazzup!"s)).Execute(empty, context);
		AssertObjectValueEqual(result, "Wazzup!"s, "");
		ASSERT_TRUE(result.TryAs<runtime::String>());
	}
	{
		...
		AssertObjectValueEqual(result, "842"s, "");
		ASSERT_TRUE(result.TryAs<runtime::String>());
	}
	{
		...
		AssertObjectValueEqual(str.Execute(closure, context), expected_output.str(), "");
	}
	{
		Stringify str(make_unique<None>());
		AssertObjectValueEqual(str.Execute(empty, context), "None"s, "");
	}

	ASSERT_TRUE(context.output.str().empty());
}

TEST(StatementTest, TestNumbersAddition)
{
	runtime::DummyContext context;
	...
	Closure empty;
	AssertObjectValueEqual(sum.Execute(empty, context), 57, "");

	ASSERT_TRUE(context.output.str().empty());
}

TEST(StatementTest, TestStringsAddition)
{
	runtime::DummyContext context;
	...
	Closure empty;
	AssertObjectValueEqual(sum.Execute(empty, context), "2334"s, "");

	ASSERT_TRUE(context.output.str().empty());
}

TEST(StatementTest, TestBadAddition)
{
	runtime::DummyContext context;

	Closure empty;

	ASSERT_THROW(Add(make_unique<NumericConst>(42), make_unique<StringConst>("4"s)).Execute(empty, context),
				  std::runtime_error);
	ASSERT_THROW(Add(make_unique<StringConst>("4"s), make_unique<NumericConst>(42)).Execute(empty, context),
				  std::runtime_error);
	ASSERT_THROW(Add(make_unique<None>(), make_unique<StringConst>("4"s)).Execute(empty, context), std::runtime_error);
	ASSERT_THROW(Add(make_unique<None>(), make_unique<None>()).Execute(empty, context), std::runtime_error);

	ASSERT_TRUE(context.output.str().empty());
}

TEST(StatementTest, TestSuccessfulClassInstanceAdd)
{
	runtime::DummyContext context;
	...
	AssertObjectValueEqual(result, "hello, world"s, "");

	ASSERT_TRUE(context.output.str().empty());
}

TEST(StatementTest, TestClassInstanceAddWithoutMethod)
{
	runtime::DummyContext context;
	...
	ASSERT_THROW(addition.Execute(empty, context), std::runtime_error);

	ASSERT_TRUE(context.output.str().empty());
}

TEST(StatementTest, TestCompound)
{
	runtime::DummyContext context;
	...
	AssertObjectValueEqual(closure.at("x"s), "one"s, "");
	AssertObjectValueEqual(closure.at("y"s), 2, "");
	AssertObjectValueEqual(closure.at("z"s), "one"s, "");
...
TEST(StatementTest, TestFields)
{
	...
	for (int i = 1, expected = 0; i < 10; expected += i, ++i)
	{
		...
		ASSERT_TRUE(obj);
		ASSERT_EQ(obj->GetValue(), expected);
...
TEST(StatementTest, TestBaseClass)
{
	vector<runtime::Method> methods;
	...
	ASSERT_EQ(cls.GetName(), "BoxedValue"s);
	{
		const auto* m = cls.GetMethod("GetValue"s);
		ASSERT_TRUE(m != nullptr);
		ASSERT_EQ(m->name, "GetValue"s);
		ASSERT_TRUE(m->formal_params.empty());
	}
	{
		const auto* m = cls.GetMethod("SetValue"s);
		ASSERT_TRUE(m != nullptr);
		ASSERT_EQ(m->name, "SetValue"s);
		ASSERT_EQ(m->formal_params.size(), 1U);
	}
	ASSERT_TRUE(!cls.GetMethod("AsString"s));
}

TEST(StatementTest, TestInheritance)
{
	vector<runtime::Method> methods;
	methods.push_back({"GetValue"s, {}, make_unique<VariableValue>(vector{"self"s, "value"s})});
	methods.push_back(
	...
	ASSERT_EQ(cls.GetName(), "StringableValue"s);
	{
		const auto* m = cls.GetMethod("GetValue"s);
		ASSERT_TRUE(m != nullptr);
		ASSERT_EQ(m->name, "GetValue"s);
	...
	}
	{
		...
		ASSERT_TRUE(m != nullptr);
		ASSERT_EQ(m->name, "AsString"s);
		ASSERT_TRUE(m->formal_params.empty());
	}
	ASSERT_TRUE(!cls.GetMethod("AsStringValue"s));
}

TEST(StatementTest, TestOr)
{
