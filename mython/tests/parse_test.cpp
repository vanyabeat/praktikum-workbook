...
TEST(ParseTest, TestSimpleProgram)
{
...
print x + y, z + n
	...
	ASSERT_EQ(context.output.str(), "9 hello, world\n"s);
}

TEST(ParseTest, TestProgramWithClasses)
{
	const string program = R"(
...
print program_name, origin, far_far_away, origin.SetX(1)
	...
	ASSERT_EQ(context.output.str(), "Classes test (0; 0) (10000; 50000) None\n"s);
}

TEST(ParseTest, TestProgramWithIf)
{
	const string program = R"(
x = 4
y = 5
...
if x > 0:
  ...
  print 'x <= 0'
	...
	ASSERT_EQ(context.output.str(), "x <= y\ny >= 0\n"s);
}

TEST(ParseTest, TestReturnFromIf)
{
	const string program = R"(
class Abs:
  def calc(n):
...
print x.calc(2)
	...
	ASSERT_EQ(context.output.str(), "2\n"s);
}

TEST(ParseTest, TestRecursion)
{
	const string program = R"(
...
print x.result
	...
	ASSERT_EQ(context.output.str(), "55\n"s);
}

TEST(ParseTest, TestRecursionAnother)
{
	const string program = R"(
class GCD:
  def __init__():
...
print x.call_count
	...
	ASSERT_EQ(context.output.str(), "17\n1\n115\n"s);
}

TEST(ParseTest, TestComplexLogicalExpression)
{
	const string program = R"(
a = 1
b = 2
...
print ok
	...
	ASSERT_EQ(context.output.str(), "False\n"s);
}

TEST(ParseTest, TestClassicalPolymorphism)
{
	const string program = R"(
class Shape:
  def __str__():
...
print r, c, t1, t2
	...
	ASSERT_EQ(context.output.str(), "Rect(10x20) Circle(52) Triangle(3, 4, 5) Wrong triangle\n"s);
}